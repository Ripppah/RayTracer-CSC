...
bool Cylinder::intersect( Ray3D& ray, const Matrix4x4& worldToModel,
        ...
        t_value_candidates.push_back(t_value2);
        t_value_candidates.push_back(t_value3);
        ...
        for(unsigned int i = 0; i<t_value_candidates.size(); i++){
                ...
                //Check if the intersection point is in the region for quadratic wall part
                if(t_value == t_value1 || t_value == t_value2 || t_value == t_value3){
                        if(t_value > 0 && point[2] >= -1 && point[2] <=1){
                                good_t_value.push_back(t_value);
                        }
                }
                //Check if the intersection point in top and bottom is inside UnitCircle 
                else if (t_value == t_valueBot || t_value == t_valueTop){
        ...
        if(ray.intersection.none || t_value < ray.intersection.t_value){
                ...
                else if (t_value == t_value1 || t_value == t_value2 || t_value == t_value3){
                        ray.intersection.normal = worldToModel.transpose()*Vector3D(intersectionPoint[0], intersectionPoint[1], intersectionPoint[2]);
                        ray.intersection.normal.normalize();
                }
                ray.intersection.none = false;
